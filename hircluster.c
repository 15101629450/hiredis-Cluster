...
static void __redisClusterSetError(redisClusterContext *cc, int type, const char *str) {
	...
	if (str != NULL) {
		...
		__redis_strerror_r(errno, ((char *)cc->errstr), sizeof(cc->errstr));
	}
}
...
static int cluster_reply_error_type(redisReply *reply)
{
	...
	if(reply->type == REDIS_REPLY_ERROR)
	{
		...
		else if((int)strlen(REDIS_ERROR_PASS) < reply->len && 
				strncmp(reply->str, REDIS_ERROR_PASS, strlen(REDIS_ERROR_PASS)) == 0)
		{
			return CLUSTER_ERR_PASS;	
		}
		else
		{
		...
		const char *ip, int port)
{
	...
	if(cc->connect_timeout){
		...
		c = redisConnect(ip, port, cc->pass);
	}
...
redisClusterContext *redisClusterContextInit(char *pass) {
	redisClusterContext *cc;
...
int redisClusterReconnect(redisClusterContext *cc)
{
...
redisClusterContext *redisClusterConnect(const char *addr, int port, char *pass)
{
	redisClusterContext *cc = NULL;
	if (!addr || strlen(addr) >= 128) {
		return NULL;
	}

	cc = redisClusterContextInit(pass);
	...
	strcpy(cc->tcp.host, addr);
	cc->tcp.port = port;

	return _redisClusterConnect(cc, addr_port);
}
		...
		const char *addrs, const struct timeval tv, int flags)
{
	...
	cc = redisClusterContextInit(NULL);
...
redisContext *ctx_get_by_node(redisClusterContext *cc, cluster_node *node)
{
	...
	{
		c = redisConnect(node->host, node->port, cc->pass);
	}
...
cluster_node *node_get_by_slot(redisClusterContext *cc, uint32_t slot_num)
{
	...
	if(error_type > CLUSTER_NOT_ERR && error_type < CLUSTER_ERR_SENTINEL)
	{
		...
		switch(error_type)
		{
			case CLUSTER_ERR_MOVED:
				...
				break;
			case CLUSTER_ERR_PASS:
				if (reply) freeReplyObject(reply);
				reply = redisCommand(c, "auth %s", cc->pass);
				if (reply) freeReplyObject(reply);
				reply = NULL;
				goto retry;
...
static void __redisClusterAsyncSetError(redisClusterAsyncContext *acc, 
	...
	if (str != NULL) {
		...
		/* Only REDIS_ERR_IO may lack a description! */
		assert(type == REDIS_ERR_IO);
		__redis_strerror_r(errno, ((char *)acc->errstr), (sizeof(acc->errstr)));
	}
}
